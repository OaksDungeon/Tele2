...
5. Откройте файл `client/src/api.js`.
6. В строку `baseURL: '***'` вместо `***` вставьте строку `http://{ip_устройства_на_котором_запускается_uvicorn_сервер}:8000` (пример: `http://192.168.0.13:8000`). Сохраните изменение в файле.
